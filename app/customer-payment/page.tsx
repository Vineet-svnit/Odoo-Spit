  ...
  useEffect(() => {
    async function loadPayments() {
      ...
      try {
        ...
        const payload = (await response.json()) as CustomerPaymentsResponse;

        if (!response.ok || !payload.success) {
          setPayments([]);
          setError(payload.error ?? "Failed to fetch customer payments");
          return;
        }

        setPayments(Array.isArray(payload.data) ? payload.data : []);
      } catch (fetchError) {
        const message = fetchError instanceof Error ? fetchError.message : "Failed to fetch customer payments";
        setPayments([]);
        setError(message);
      } finally {
        setIsLoading(false);
      ...
      <section className="mx-auto w-full max-w-6xl rounded-3xl border border-black/10 bg-white/90 p-6 shadow-[0_28px_80px_-40px_rgba(0,0,0,0.38)] backdrop-blur md:p-8">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-semibold tracking-tight text-zinc-900">Customer Payments</h1>
            <p className="mt-2 text-sm text-zinc-600">All previous customer transactions.</p>
          </div>
        ...
        {isLoading ? <p className="text-sm text-zinc-600">Loading customer payments...</p> : null}
        {error ? <p className="rounded-xl bg-red-100 px-4 py-3 text-sm text-red-700">{error}</p> : null}

        {!isLoading && !error ? (
          payments.length === 0 ? (
            <p className="text-sm text-zinc-600">No customer payments found.</p>
          ) : (
              ...
              <table className="min-w-full divide-y divide-zinc-200 text-sm">
                ...
                <tbody className="divide-y divide-zinc-200">
                  {payments.map((payment) => (
                    <tr key={payment.customerPaymentId} className="hover:bg-emerald-50/40">
                      <td className="px-4 py-3 font-medium text-zinc-900">
                        <Link href={`/customer-payment/${payment.customerPaymentId}`} className="hover:text-emerald-700">
                          {payment.payId}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-zinc-700">{payment.partnerName}</td>
                      <td className="px-4 py-3 text-zinc-700">{formatDate(payment.paymentDate ?? payment.createdAt)}</td>
